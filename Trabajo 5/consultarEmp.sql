CREATE OR REPLACE PROCEDURE ConsultaEmp(numBusqueda IN NUMBER) IS
...
begin
  ...
  loop
    if nodos(nodoInicio)(0) is null then
      dbms_output.put_line('numnodo inicio: '||nodoInicio||', depE: No tiene');
    else
      dbms_output.put_line('numnodo inicio: '||nodoInicio||', depE: '||nodos(nodoInicio)(0));
    end if;
    -- existe
    nodoFin := nodos(nodoInicio)(puntero);
    if nodos(nodoFin)(0) is null then
      dbms_output.put_line('numnodo fin: '||nodoFin||', depE: No tiene');
    else
      dbms_output.put_line('numnodo fin: '||nodoFin||', depE: '||nodos(nodoFin)(0));
    end if;
    ...
    if nodos(nodoFin)(0) = numBusqueda then
      ...
      nodoInicio := nodoFin;
      dbms_output.put_line('Como el depE del nodo fin es menor que '||numBusqueda||' entonces se actualiza nodo inicio por nodo fin');
    else
      puntero := puntero-1;
      dbms_output.put_line('Como el depE del nodo fin NO es menor que '||numBusqueda||' entonces NO se actualiza nodo inicio por nodo fin');
    end if;
