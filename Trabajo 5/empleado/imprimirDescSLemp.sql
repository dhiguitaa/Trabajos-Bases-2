CREATE OR REPLACE PROCEDURE ImprimirDescSLemp IS
...
begin
    SELECT count(*) into indicePtrback FROM indexempskip;

    WHILE indicePtrback > 0 loop

        FOR fila IN (SELECT * FROM indexempskip WHERE numnodo = indicePtrback) LOOP

            dbms_output.put_line('NODO ' || fila.numnodo);

            -- Imprimir datos dpto
            IF (fila.depE IS NULL) THEN
                dbms_output.put_line('Datos dpto: No tiene');

            ELSE
                dbms_output.put_line('Datos dpto: ' || fila.depE);
            END IF;


            -- Informacion de cada empleado perteniciente al departamento
            for filaInfoEmp in (SELECT * FROM TABLE(fila.infoEmp)) loop
                dbms_output.put_line(filaInfoEmp.id  || ', ' || filaInfoEmp.nombre);
            end loop;
