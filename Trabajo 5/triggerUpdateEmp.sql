...
BEGIN
    ...
    if codigosViejos.count != 0 then
      ...
      -- verificar si ya existe el nuevo depE
      existeDep := false;
      for i in codigosViejos.first .. codigosViejos.last loop
        if new_dep = codigosViejos(i) then
          existeDep := true;
        end if;
      end loop;
      if existeDep then
        nuevaAltura := cantidadPunterosViejos(new_dep);
      else 
        nuevaAltura := round(DBMS_RANDOM.VALUE(1,maxPtrs));
        cantidadPunterosViejos(new_dep) := nuevaAltura;
      end if;
