...
public class punto3 extends JFrame implements ActionListener{//implementando el listener de eventos
...
JPanel panel;
  Border border;
  public punto3(){//constructor de la clase  
    ...
    add(panel);
    setTitle("Regla 5 (Ojo, por favor procure que los atributos entre tablas se llamen diferente");
  ...
  punto3 gj = new punto3();//uso de constructor para la ventana
  }
  ...
  public void actionPerformed(final ActionEvent e) {// sobreescribimos el metgetLocaleso del listener
    ...
    String clave2 =  " ";
    for (int i = 0; i < splitT2.length; i++){
        for (int j = 0; j <splitT3.length; j++){
          if(splitT3[j].equals(splitT2[i])){
            clave2 = splitT1[i];
            break;
          }
        }
    ...
    for (int i = 0; i < splitS13.length; i++) {
      ...
      for (int l = 0; l < splitT3.length; l++) {
        if (splitS12[i].contains(splitT3[l]) && (splitT3[l].equals(clave2))== false) {
            palabra += splitT3[l] +  "," ;
        }
      }
      if (!(palabra.equals(" "))) {
    ...
    for (int i = 0; i < splitS3.length; i++) {
      ...
      for (int l = 0; l < splitT3.length; l++) {
        if (splitS3[i].contains(splitT3[l]) && (splitT3[l].equals(clave2))== false) {
            palabra += splitT3[l] +  "," ;
        }
      }
      String[] conjunto = palabra.split(",");
